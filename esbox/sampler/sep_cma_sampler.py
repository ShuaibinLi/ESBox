...
__all__ = ['SepCMASampler']


class SepCMASampler(BoundedSampler):
    """Class implementing the Sep-CMA sampler.
    Args:
    ...
    def __init__(self, weights_size, bounds=None, n_max_resampling=100, seed=1234, sigma=None, cov=None):
        ...
        if cov is None:
            ...
            self._C = cov
        self._sigma = sigma
        print("Initialization of Sep-CMA sampler complete.")
    ...
    def sample(self, weights, sample_batch=1, covariance_matrix=None, sigma=None):
        ...
        if sigma is not None:
            self._sigma = sigma
        D = self._eigen_decomposition(self._C)

        sample_noise_index = []
        sampled_x = []
        for _ in range(sample_batch):
            for i in range(self._n_max_resampling):
                noise_idx, x = self._sample_solution(D)
    ...
    def _eigen_decomposition(self, C):
        D = np.sqrt(np.where(C < 0, EPS, C))
        return D

    def _sample_solution(self, D):
