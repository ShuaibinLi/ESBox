...
# https://github.com/PaddlePaddle/PARL/blob/develop/parl/env/atari_wrappers.py
# Atari wrapper for single agent
...
class MonitorEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        self._total_steps += 1
        if done:
            epinfo = {"r": self._current_reward, "l": self._num_steps, "t": round(time.time() - self.tstart, 6)}
...
class FrameStack(gym.Wrapper):
    def __init__(self, env, k, obs_format='NHWC'):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=obs_shape, dtype=env.observation_space.dtype)
...
class TestEnv(gym.Wrapper):
    def __init__(self, env, test_episodes=3):
        ...
        self._end_episode = len(self._monitor.get_episode_rewards()) + test_episodes
...
def wrap_deepmind(env, dim=84, framestack=True, obs_format='NHWC', test=False, test_episodes=3):
